 ...
 * 
 */
var baseUri = "http://localhost:8081/jaxrsWebapp01/rest";
...
function saveEmp(empData, responseHandler) {
	...
	const uri = `${baseUri}/employeeService/save`;
...
function deleteEmp(id,responseHandler) {
	let result = null;
	if (id != null && id != "") {
		const uri = `${baseUri}/employeeService/delete/${id}`;
...
function updateEmp(empData, responseHandler) {
	...
	if (empData) {
		const jsonString = JSON.stringify(empData);
		const uri = `${baseUri}/employeeService/update`;
